...
def draw_copy_menu():
    ...
    menu_display.blit(pygame.transform.scale(image_play_button_click, (
    image_play_button_click.get_width() * 5, image_play_button_mouse_on.get_height() * 5)),
                      (SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 - 50))
    menu_display.blit(pygame.transform.scale(image_exit_button_unclick, (
    image_exit_button_unclick.get_width() * 5, image_exit_button_unclick.get_height() * 5)),
...
def draw_copy_game(player_pos_x, player_pos_y):
    ...
    # blit la tête verte des robots sauvés
    ...
    for particle in deathblock_particles_group:
        particle.update()
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed):
        ...
        self.animation_list = []
        animation_types = ['Idle', 'Run', 'Stun', 'Death', 'Hurt', 'Dance', 'Save', 'Attack', "Run_angry"]  # = 8
        self.frame_index = 0
        self.action = 0
    ...
    def move(self, moving_left, moving_right):
        ...
        for tile in world.obstacle_list:
            if tile[1].colliderect(self.rect.x + dx, self.rect.y, self.width, self.height) and tile[2] != 44:
                if tile[1].colliderect(self.collision_rect.x + dx, self.collision_rect.y, self.collision_rect.width,
                                       self.collision_rect.height):
                    dx = 0

            if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.width, self.height) and tile[2] != 44:
        ...
        # dont let ennemy fall and make them rebound

            if tile[2] == 44:
                if tile[1].colliderect(self.rect.x + dx, self.rect.y, self.width, self.height) and self.ready_to_attack == False:
                    if tile[1].colliderect(self.collision_rect.x + dx, self.collision_rect.y, self.collision_rect.width,
                                           self.collision_rect.height):
                        self.direction *= -1
                        dx *= -1
                ...
                if self.save == False:
                    if self.stun_timer == self.max_stun_cooldown:  # 240 = 4 secondes / si il n'est pa sauvé au bout de 4 secondes
                        if self.stun_sound == 0:
                            ...
                            self.update_action(3)  # il explose
                            explosion_fx.play()
    ...
    def update_animation(self):
        ...
        if self.action == 0:  # IDLE
            ...
            if pygame.time.get_ticks() - self.update_time > ANIMATION_COOLDOWN + 2:
                self.update_time = pygame.time.get_ticks()  # on remet a jour le temps
                self.frame_index += 1
        ...
        if self.frame_index >= len(
                self.animation_list[self.action]):  # si l'index dépasse ou est égale a la dernière frame de la liste
    ...
    def ai(self):
        if self.stun == False and player.alive and pause_game == False:
            ...
            if self.idling == False and random.randint(1,
                                                       200) == 1 and self.hurt == False and self.ready_to_attack == False:
                self.idling = True
            ...
            if self.vision.colliderect(
                    player.collision_rect) and self.ready_to_attack == True and self.hurt == False:  # and self.hurt == True # remplacer par collide rect ?
                ...
                if self.idling == False and self.hurt == False:
                    ...
                    if self.hurt_cooldown == 0:
                        ...
                        if self.ready_to_attack == True:
                            self.update_action(8)  # pour qu'ils courent avec yeux rouges
                        else:
                            self.update_action(1)  # pour qu'ils courent normal

                        self.move_counter += 1

                    # update ai vision as the enemy move
                    self.vision.center = (
                    (self.collision_rect.centerx + 30 * self.direction), self.collision_rect.centery)
                    # pygame.draw.rect(screen,RED, (self.vision.x - tile_bg_scroll[0], self.vision.y - tile_bg_scroll[1], self.vision.width, self.vision.height))
    ...
    def draw(self):
        ...
        #pygame.draw.rect(screen, BLUE, (self.deathblock_rect.x - true_scroll[0], self.deathblock_rect.y - true_scroll[1], self.deathblock_rect.width,self.deathblock_rect.height), 2)
...
class Clyde(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, health):
        ...
        self.animation_list = []
        animation_types = ['Idle', 'Run', 'Jump', 'Death', 'Attack', 'Save', 'Hurt', 'Shoot Pos', 'Dash', 'WakeUp',
                           'Shoot Front', 'Shoot Up', 'Aim Top']
        self.frame_index = 0
        self.action = 0
        self.update_time = pygame.time.get_ticks()
        # load all images for the players
    ...
    def attack(self):
        ...
        for robot_enemy in enemy_group:
            if pygame.sprite.collide_rect(self, robot_enemy):
                if pygame.sprite.collide_mask(self, robot_enemy) and self.attack_hit.colliderect(
                        robot_enemy.collision_rect):
    ...
    def save(self):
        ...
        for robot_enemy in enemy_group:
            if robot_enemy.stun_timer >= 0 and robot_enemy.stun_timer <= 300 and robot_enemy.stun == True:
                if pygame.sprite.collide_rect(self, robot_enemy):
                    if pygame.sprite.collide_mask(self, robot_enemy) and self.save_hit.colliderect(
                            robot_enemy.collision_rect):  # and collision rectangle vert bonne directiono
                        self.num_saved += 1
    ...
    def move(self, moving_left, moving_right, dash, wake_up):
        ...
        # check for collision
        for tile in world.obstacle_list:
            if tile[2] != 44: # pour ne pas être concerné par le bloc vert rebound
        ...
        if pause_game == False:  # pour que la caméra arrète de bouger quand le jeu est en pause
            if wake_up == False:
                true_scroll[0] += (player.rect.x - true_scroll[0] - (
                            400 - player.image.get_width())) / 10  # jouez avec 400 ou 320 pour monter / descendre / droite / gauche caméra
    ...
    def shoot(self):
        if (player.ready_to_shoot_up or player.ready_to_shoot_front) and self.shoot_cooldown == 0 and self.ammo > 0:
            ...
            if player.ready_to_shoot_up:
                ...
                bullet = Bullet(
                    player.collision_rect.centerx + (0.75 * player.collision_rect.size[0] * player.direction),
                    player.rect.centery, player.direction, 1, 'front')
    ...
    def draw(self):
        screen.blit(pygame.transform.flip(self.image, self.flip, False),
                    (self.rect.x - true_scroll[0], self.rect.y - true_scroll[1]))
        #pygame.draw.rect(screen,RED,(self.rect.x - true_scroll[0], self.rect.y - true_scroll[1], self.image.get_width(), self.image.get_height()),2)
        #pygame.draw.rect(screen, PINK, (self.collision_rect.x - true_scroll[0], self.collision_rect.y - true_scroll[1], self.collision_rect.width,self.collision_rect.height), 2)
        ...
        for y, row in enumerate(data):
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    if tile >= 0 and tile <= 26 or tile == 39 or tile == 41 or tile == 44:
                        self.obstacle_list.append(tile_data)

                        if tile == 44: # pour que le bloc vert qui permet de faire rebondir les ennemies soit invisible
                            tile_data[0].set_colorkey((23,95,154))
                        ...
                        if LEVEL > 1:
                            ...
                            player = Clyde('player', x * TILE_SIZE, y * TILE_SIZE, 1.8, 5, 1, 5)
                        ...
                        roof_ennemy = Roof_enemy("roof enemy", (x * TILE_SIZE) + TILE_SIZE, y * TILE_SIZE + 18, 3)
                        ...
                        particles = Deathblock_particles("Particles",x * TILE_SIZE + 21, y * TILE_SIZE + 17, 3)
                        deathblock_particles_group.add(particles)
...
class Deathblock_particles(pygame.sprite.Sprite):
    def __init__(self, char_type, x,y,scale):
        ...
        self.animation_list = []
        animation_types = ['Deathblock_particles']
        self.frame_index = 0
        self.action = 0
        self.update_time = pygame.time.get_ticks()
        ...
        self.death_rect = pygame.Rect(0,0,37,2)
    ...
    def update(self):
        ...
        self.death_rect.midbottom = self.rect.midbottom

        if self.death_rect.colliderect(player.collision_rect):
            player.health = 0

        for enemy in enemy_group:
            if self.death_rect.colliderect(enemy.deathblock_rect):
                enemy.stun = True
                enemy.save = False
                enemy.stun_timer = enemy.max_stun_cooldown  # 240 = 4 secondes / si il n'est pa sauvé au bout de 4 secondes
                enemy.stun_sound = 0
                enemy.health = 0
        ...
        #pygame.draw.rect(screen, RED, (self.death_rect.x - tile_bg_scroll[0], self.death_rect.y - tile_bg_scroll[1], self.death_rect.width, self.death_rect.height), 2)
...
class Roof_enemy(pygame.sprite.Sprite):
    ...
    def attack(self):
        ...
        if self.action == 0 and self.frame_index == 5 and self.shoot_cooldown == 80 and pause_game == False:
            grenade = Grenade(self.rect.centerx, self.rect.bottom - 2)
...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        for tile in world.obstacle_list:
            if tile[1].colliderect(self.rect) and tile[2] != 44: # != 44 pour ne pas que le missile s'arrete s'il croise un bloc rebound invisible
                self.update_action(1)
        ...
        screen.blit(pygame.transform.rotate(self.image, self.flip),
                    (self.rect.x - tile_bg_scroll[0], self.rect.y - tile_bg_scroll[1]))
...
def reset_level():  # vide tous les groupes et réinitialise la map avec des -1
    ...
    deathblock_particles_group.empty()
...
while run:
    ...
    if start_game == False:
        ...
        for enemy in enemy_group:
            ...
            if (player.rect.x - enemy.rect.x >= -850 and player.rect.x <= enemy.rect.x) or (
                    enemy.rect.x - player.rect.x >= -850 and player.rect.x >= enemy.rect.x) or enemy.ready_to_attack == True or LEVEL == MAX_LEVELS:
                if (0 <= player.rect.y - enemy.rect.y <= 400 and player.rect.y >= enemy.rect.y) or (player.rect.y <= enemy.rect.y and 0 <= enemy.rect.y - player.rect.y <= 400):
                    enemy.ai()
                    enemy.draw()
                    enemy.update()
                    enemy.check_state()
        ...
        for roof_enemy in roof_ennemy_group:
            if (player.rect.x - roof_enemy.rect.x >= -600 and player.rect.x <= roof_enemy.rect.x) or (
                    roof_enemy.rect.x - player.rect.x >= -600 and player.rect.x >= roof_enemy.rect.x) or LEVEL == MAX_LEVELS:
                if (0 <= player.rect.y - roof_enemy.rect.y <= 400 and player.rect.y >= roof_enemy.rect.y) or (
                        player.rect.y <= roof_enemy.rect.y and 0 <= roof_enemy.rect.y - player.rect.y <= 400):
                    roof_enemy.attack()
                    roof_enemy.update()
                    roof_enemy.check_state()
        ...
        true_scroll, dash = player.move(moving_left, moving_right, dash,
                                        wake_up)  # on recupere le niveau et la valeur de scroll
                ...
                if blit_button_once == True:
                    ...
                    for robot_enemy in enemy_group:
                        if robot_enemy.stun == True and robot_enemy.stun_timer <= 300 and robot_enemy.save != True and robot_enemy.alive != False:
                            screen.blit(ibutton_list[ibutton_index], (
                            robot_enemy.collision_rect.x - tile_bg_scroll[0] + 15 * robot_enemy.direction,
                            robot_enemy.collision_rect.y - tile_bg_scroll[1] - 80))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if (event.key == K_UP or event.key == K_w) and player.alive and shoot_front == False and shoot_up == False:
                if cant_play == False:
                    if player.in_air == False:
                        player.jump = True

            if event.key == K_SPACE and player.alive and shoot_front == False and shoot_up == False:
                if cant_play == False:
